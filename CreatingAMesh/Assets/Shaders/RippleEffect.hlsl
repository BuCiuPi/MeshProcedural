...
out float3 TangentOut
	...
	float2 derivative = (2.0 * PI * Amplitude * Period * cos(f) / max(d, 0.0001)) * p.xz;
	TangentOut = float3(1.0, derivative.x, 0.0);
	NormalOut = cross(float3(0.0, derivative.y, 1.0), TangentOut);
}